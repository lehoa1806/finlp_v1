...
class Announcement2SlackPost(announcement2slack_post.Announcement2SlackPost):
    ...
    def channels(self) -> Dict:
        return self.setting.vietnam_channels
...
VIETNAM_ANNOUNCEMENT_CONSUMER = HybridConsumer(
    consumers=[
        HybridConsumer(
            pipeline=Pipeline(
            ...
            consumers=[
                PostgresBatchInsert(
                    table_name='vietnam_announcements',
                    batch_size=10,
                ),
            ],
        ),
...
VIETNAM_NEWS_CONSUMER = HybridConsumer(
    consumers=[
        HybridConsumer(
            pipeline=Pipeline(
                stage=FilteringStage(
...
class AnnouncementScraperJob(ScraperJob):
    ...
    def consumer(self) -> Consumer:
        return VIETNAM_ANNOUNCEMENT_CONSUMER
